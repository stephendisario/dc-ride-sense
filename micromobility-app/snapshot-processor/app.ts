...
export const lambdaHandler = async (
  ...
  const {
    ...
    month: lastMonth,
    day: lastDay,
  } = parseDateString(lastHourTimestamp);
  ...
  for (const { feed, provider } of feeds) {
    try {
      ...
      for (const zoneId of allZoneIds) {
        ...
        const delta = hadPrevSnapshot
          ? density - lastSnapshotBikeIds.length
          : 0;

        let churn = 0;
        if (hadPrevSnapshot && PROVIDERS_WITH_CHURN.includes(provider)) {
          const uniqueOld = lastSnapshotBikeIds.filter(
            (id) => !bikeIds.includes(id)
          );
          const uniqueNew = bikeIds.filter(
            (id) => !lastSnapshotBikeIds.includes(id)
          );
